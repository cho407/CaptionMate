...
extension NSSavePanel {
    /// Presents an NSSavePanel with the specified options and returns the selected file URL and selected content type asynchronously.
    /// - Parameters:
    ///   - allowedFileTypes: An array of allowed UTTypes (e.g. [UTType.srt, UTType.vtt, UTType.json]).
    ///   - defaultFileName: The default file name (without extension).
    ///   - title: The panel title.
    /// - Returns: A tuple containing the URL selected by the user and the selected UTType, or (nil, nil) if cancelled.
